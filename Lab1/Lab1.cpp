...
int main()
{
    ...
    */
    ...
    /*
    Глобальная область видимости (Global scope): переменные, объявленные вне всех функций, 
    имеют глобальную область видимости и могут быть доступны в любой части программы.

    Локальная область видимости (Local scope): переменные, объявленные внутри функции, 
    имеют локальную область видимости и доступны только внутри этой функции.

    Область видимости блока (Block scope): переменные, объявленные внутри блока (например, в цикле или условном операторе), 
    имеют область видимости только внутри этого блока.

    Область видимости параметра функции (Function parameter scope): переменные, объявленные как параметры функции, 
    имеют область видимости только внутри этой функции.
    */

    // Какие области видимости в параллельных программах
    // Показать какая из обалстей видимости где находится на слайде 6 в 3 презентации
